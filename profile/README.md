...
| [dca-marketplace](https://github.com/domain-centric-development/dca-marketplace) | A Claude Code plugin marketplace: `dca-core` (the project description, a new project from a generator, the published packages in an existing one, the modelling craft, the DCA review, the knowledge catalog as a skill), `dca-craft` (TDD, clean code, ADRs, end-user tests, glossary, context map and three general review perspectives) and `dca-factory` (the delivery pipeline) — Java/Spring and .NET/C#. |
